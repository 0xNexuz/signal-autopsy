...
const firstRow = (value) => Array.isArray(value) ? value[0] || {} : Array.isArray(value?.data) ? value.data[0] || {} : value?.data || value || {};
const candleRows = (value) => Array.isArray(value) && Array.isArray(value[0]) ? value : Array.isArray(value?.data) ? value.data : [];
...
function volatilityFromCandles(rows) {
  const closes = rows.map((row) => Number(row[4])).filter(Boolean).reverse();
  if (closes.length < 3) return 40;
...
function depthMetrics(book, price, quoteVolume) {
  ...
  if (!asks.length || !bids.length) {
    ...
    return { quality: round(clamp(Math.log10(proxyNotional) * 14, 22, 74), 1), notional: round(proxyNotional), status: "SIMULATED", basis: "volume proxy" };
  }
  ...
  return { quality: round(clamp(Math.log10(Math.max(notional, 1)) * 14 + 18 - spreadBps * 1.5, 5, 100), 1), notional: round(notional), status: "REAL", basis: "Reality order book" };
}

export async function getRealityMarket(symbol) {
  ...
  if (!price) throw new Error("Bitget returned no Reality price");
  ...
  const candles = await bitgetRequest("/api/v3/market/candles", { query: { category: "SPOT", interval: "1D", limit: "30", symbol, type: "market" } });
  let book = {};
  let bookStatus = "SIMULATED";
  if (hasBitgetCredentials()) {
    try {
      ...
      bookStatus = "SIMULATED";
    }
  }
  const depth = depthMetrics(book, price, quoteVolume);
  if (bookStatus !== "REAL") depth.status = "SIMULATED";
  return { symbol, observedAt: new Date().toISOString(), price: round(price, 4), drift24hPercent: round(drift), quoteVolume: round(quoteVolume), volatilityRisk: volatilityFromCandles(candleRows(candles.data)), depthQuality: depth.quality, depthNotional: depth.notional, session: sessionRisk(), sources: { ticker: { status: "REAL", provider: "Bitget Agent Hub market intent / UTA v3" }, candles: { status: "REAL", provider: "Bitget UTA v3" }, liquidity: { status: depth.status, provider: depth.basis } } };
}
